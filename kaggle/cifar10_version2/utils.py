...
def evaluate_test_with_gpus_ema(ema: EMA, net: nn.Module, test_iter) -> float:
...
def gen_cifar10_train_val_text(datasets_path: str, val_rate: float = 0.1) -> None:
    ...
    train_list = [join(pwd, datasets_path, "train", file_name)
                  for file_name in file_name_list[0: int(totle_len * (1 - val_rate))]]
    val_list = [join(pwd, datasets_path, "train", file_name)
                for file_name in file_name_list[int(totle_len * (1 - val_rate)):]]
